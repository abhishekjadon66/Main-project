...
export async function getServerSideProps(context) {
  ...
  let products = await Product.find({ category: "Services" });
  let Services = {};
  for (let item of products) {
    if (item.title in Services) {
      if (
        !Services[item.title].color.includes(item.color) &&
        item.availableQty > 0
      ) {
        Services[item.title].color.push(item.color);
      }
      if (
        !Services[item.title].size.includes(item.size) &&
        item.availableQty > 0
      ) {
        Services[item.title].size.push(item.size);
      }
    } else {
      Services[item.title] = JSON.parse(JSON.stringify(item));
      if (item.availableQty > 0) {
        Services[item.title].color = [item.color];
        Services[item.title].size = [item.size];
      }
      else {
          Services[item.title].color = [];
          Services[item.title].size = [];
      }
    }
  }

  return {
    props: { products: JSON.parse(JSON.stringify(Services)) },
  };
}
