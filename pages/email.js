...
export async function getServerSideProps(context) {
  ...
  let products = await Product.find({ category: "Email" });
  let Email = {};
  for (let item of products) {
    if (item.title in Email) {
      if (
        !Email[item.title].color.includes(item.color) &&
        item.availableQty > 0
      ) {
        Email[item.title].color.push(item.color);
      }
      if (
        !Email[item.title].size.includes(item.size) &&
        item.availableQty > 0
      ) {
        Email[item.title].size.push(item.size);
      }
    } else {
      Email[item.title] = JSON.parse(JSON.stringify(item));
      if (item.availableQty > 0) {
        Email[item.title].color = [item.color];
        Email[item.title].size = [item.size];
      }
      else {
          Email[item.title].color = [];
          Email[item.title].size = [];
      }
    }
  }

  return {
    props: { products: JSON.parse(JSON.stringify(Email)) },
  };
}
