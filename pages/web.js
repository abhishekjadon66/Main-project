...
export async function getServerSideProps(context) {
  ...
  for (let item of products) {
    if (item.title in Web) {
      if (
        !Web[item.title].color.includes(item.color) &&
        item.availableQty > 0
      ) {
        Web[item.title].color.push(item.color);
      }
      if (
        !Web[item.title].size.includes(item.size) &&
        item.availableQty > 0
      ) {
        Web[item.title].size.push(item.size);
      }
    } else {
      Web[item.title] = JSON.parse(JSON.stringify(item));
      if (item.availableQty > 0) {
        Web[item.title].color = [item.color];
        Web[item.title].size = [item.size];
      }
      else {
          Web[item.title].color = [];
          Web[item.title].size = [];
      }
    }
  }
  ...
  return {
    props: { products: JSON.parse(JSON.stringify(Web)) },
  };
}
